...
interface ReportPreviewProps {
  ...
  onUnlock: () => void; // Função que redireciona para o checkout
}
...
export const ReportPreview: React.FC<ReportPreviewProps> = ({ transactionId, onUnlock }) => {
  ...
  useEffect(() => {
    // Carrega o gancho persuasivo assim que a tela monta
    const loadPreview = async () => {
      try {
        const data = await fetchReportPreview(transactionId);
        setPreviewText(data.previewText);
      } catch (error) {
        // Fallback de segurança caso a IA demore ou falhe, para não perder a venda
      ...
      <div className="max-w-3xl w-full space-y-8 animate-fade-in">
        ...
        <div className="text-center space-y-4">
          <div className="inline-flex items-center justify-center p-3 bg-green-900/30 border border-green-500/30 rounded-full mb-2 shadow-[0_0_15px_rgba(34,197,94,0.3)]">
            ...
            <span className="ml-2 text-green-400 font-semibold text-sm tracking-wider uppercase">Análise Processada com Sucesso</span>
          </div>
          
          <h1 className="text-3xl md:text-5xl font-extrabold text-white leading-tight">
            Resultado Preliminar: <span className="text-transparent bg-clip-text bg-gradient-to-r from-amber-400 to-orange-500">Perfil Detectado</span>
          </h1>
          
          <p className="text-slate-400 text-lg max-w-xl mx-auto leading-relaxed">
            Nossa IA cruzou suas respostas com mais de 10.000 perfis executivos e encontrou um padrão revelador.
          </p>
        </div>

        {/* Card Principal com o Gancho */}
        <Card className="border-amber-500/40 bg-gradient-to-b from-slate-900 to-slate-950 shadow-2xl shadow-amber-900/10">
          <CardContent className="p-6 md:p-10">
            
            {/* Texto da IA (O Gancho) */}
            <div className="mb-10">
              ...
              {loading ? (
                ...
                <div className="text-xl md:text-2xl text-slate-200 font-medium italic leading-relaxed border-l-4 border-amber-500 pl-6 py-2 bg-amber-500/5 rounded-r-lg">
                  "{previewText}"
                </div>
              )}
            </div>

            {/* Seção Bloqueada (FOMO) */}
            <div className="relative rounded-2xl border border-slate-800 bg-slate-900 overflow-hidden group mt-8">
              ...
              <div className="p-8 filter blur-sm opacity-30 select-none pointer-events-none bg-slate-900">
                ...
                <ul className="space-y-4 text-slate-300">
                  ...
                  <li className="flex items-center gap-2"><div className="w-2 h-2 bg-red-500 rounded-full"></div> Mapa de competências comparativo...</li>
                </ul>
              </div>

              {/* Overlay de Cadeado e Call to Action */}
              <div className="absolute inset-0 z-10 flex flex-col items-center justify-center text-center bg-gradient-to-t from-slate-950 via-slate-950/90 to-transparent p-6">
                ...
                <h3 className="text-2xl md:text-3xl font-bold text-white mb-2">
                  O relatório completo está pronto
                </h3>
                
                <p className="text-slate-400 mb-8 max-w-md text-sm md:text-base">
                  Não deixe este padrão oculto sabotar seu crescimento por mais tempo. Acesse seu plano de ação personalizado agora.
                </p>
