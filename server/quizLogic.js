 ...
 * @param {Array<{id: number; text: string; module: string}>} questions - A lista de perguntas que foi apresentada ao usuário.
...
export function calculateScores(answers, questions) {
  ...
  const calculatedScores = Object.entries(answers).reduce((acc, [questionId, score]) => {
    // CORREÇÃO CRÍTICA AQUI: Compara os IDs como strings para evitar erros de tipo.
  ...
  for (const moduleKey in calculatedScores) {
    const totalQuestionsInModule = questionsPerModule[moduleKey] || 0;
    const maxScorePossible = totalQuestionsInModule * 5; // pontuação máxima por pergunta é 5
  ...
  console.log('[LOGIC] Pontuações calculadas e normalizadas:', calculatedScores);
